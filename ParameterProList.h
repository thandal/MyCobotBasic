#ifndef PARAMETER_LIST_PRO_H
#define PARAMETER_LIST_PRO_H

#include <array>
#include <map>

namespace myCobotProDefine {

#define SYSTEM_VERSION		0x01

#define MYCOBOTPRO_VERSION			101
#define PI					3.14159265
#define COEFFICIENT				(180.0 / PI)

	// DH parameters
	const float a2 = -136.35;
	const float a3 = -80.00;

	const float alpha_1 = PI / 2;
	const float alpha_4 = PI / 2;
	const float alpha_5 = -PI / 2;

	const float d1 = 170.46;
	const float d4 = 85.00;
	const float d5 = 80.00;
	const float d6 = 48.90;
	...
	// servo speed
	const float maximum_large_servo_speed = 3000;	//2000?
	const float maximum_small_servo_speed = 3000; // 3000?

	// isInPosition offset
	const float is_in_position_offset = 5;
	const float is_in_poseture_offset = 0.3;
	const float is_in_angles_offset = 0.05;
	...
	struct JointRanges
	{
		...
		//jonit 2 range
		float j_2_min = -165 / COEFFICIENT;
		float j_2_max = 165 / COEFFICIENT;
		//jonit 3 range
		...
		//jonit 6 range
		float j_6_min = -165 / COEFFICIENT;
		float j_6_max = 165 / COEFFICIENT;
	};
